    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground">ეს ცხრილი მთლიანად ავტომატურადაა გამოთვლილი «პროექტის მონაცემები» და «ფინანსური დაშვებები» ფურცლების საფუძველზე.</p>
        </CardHeader>
            ...
            <TableBody>
              {eco.units.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell className={"text-right " + computedCls}>{fmtUsd(r.priceNoVat)}</TableCell>
                  <TableCell className={"text-right " + computedCls}>{fmtUsd(r.vat)}</TableCell>
                  <TableCell className={"text-right " + computedCls}>{fmtUsd(r.bankGuarantee)}</TableCell>
                  <TableCell className={"text-right font-semibold " + computedCls}>{fmtUsd(r.finalPrice)}</TableCell>
              ...
              <TableRow className="bg-muted font-semibold">
                ...
                <TableCell className={"text-right " + computedCls}>{fmtUsd(eco.totals.vat)}</TableCell>
                <TableCell className={"text-right " + computedCls}>{fmtUsd(eco.totals.bankGuarantee)}</TableCell>
                <TableCell className={"text-right " + computedCls}>{fmtUsd(eco.totals.finalPrice)}</TableCell>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mt-3 flex items-center justify-between rounded border p-3">
            ...
            <span className={"font-semibold " + computedCls}>{fmtPct(eco.report.totalMarginPct)}</span>
          </div>
        </CardContent>
      </Card>
    </div>
